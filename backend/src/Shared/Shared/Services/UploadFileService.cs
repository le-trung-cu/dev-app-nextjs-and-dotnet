...
public class UploadFileService(IWebHostEnvironment webHostEnvironment)
  : IUploadFileService
{
  ...
  public async Task<string> SaveFileAsync(IFormFile file, string directory, string fileName, CancellationToken cancellationToken = default)
  {
    var rootPath = webHostEnvironment.WebRootPath;
    var dateFolder = DateTime.UtcNow.ToString("yyyy-MM-dd");
    var uploadPath = Path.Combine(rootPath, directory, dateFolder);
